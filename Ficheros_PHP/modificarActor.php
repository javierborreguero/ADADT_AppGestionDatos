...
require 'jsonEsperadoModificarActor.php';
...
if(isset($parameters)){
	...
	if(JSONCorrectoModificarActor($mensajeRecibido)){

		$repesentante = $mensajeRecibido["actorModificar"];

		$id = $actor["Id"];
		$nombre = $actor["Nombre"];
		$nacionaliad = $actor["Nacionalidad"];
		$edad = $actor["Edad"];
		$residencia = $actor["Residencia"];
		$idpelicula = $pelicula["Pelicula"];

		if($idpelicula == "null"){
			$query  = "UPDATE actores SET Nombre = '$nombre' ,Nacionalidad = '$nacionaliad', Edad  ='$edad',Residencia  = '$residencia',Pelicula  = null WHERE Id='$id'";
		}else{
			$query  = "UPDATE actores SET Nombre = '$nombre' ,Nacionalidad = '$nacionaliad', Edad  ='$edad',Residencia  = '$residencia',Pelicula  = null WHERE Id='$id'";
		}
		...
		if (isset ( $result ) && $result) { // Si pasa por este if, la query está está bien y se ha insertado correctamente
			...
			$arrMensaje["mensaje"] = "Actor actualizado correctamente";
