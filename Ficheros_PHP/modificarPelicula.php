...
require 'jsonEsperadoModificarPelicula.php';
...
if(isset($parameters)){
	...
	if(JSONCorrectoModificarRepresentante($mensajeRecibido)){

		$pelicula = $mensajeRecibido["peliculaModificar"];

		$id = $pelicula["id"];
		$nombre = $pelicula["nombre"];
		$descripcion = $pelicula["desrcipcion"];


		$query  = "UPDATE representantes SET Nombre = '$nombre' ,Descripcion = '$descripcion' WHERE Id= '$id' ";

		$result = $conn->query ( $query );

		if (isset ( $result ) && $result) { // Si pasa por este if, la query está está bien y se ha insertado correctamente
			$arrMensaje["estado"] = "ok";
			$arrMensaje["mensaje"] = "Pelicula actualizada correctamente";
